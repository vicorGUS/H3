...
void DMC_6dim(int I, int i0, int* N, int Nmax, double* pos, double* ET, double* ET_avg, double dtau, double gamma){
    ...
    for (int i=0; i<I; i++){    
        ...
        for (int j=0; j<N[i]; j++){
            ...
            //Drift part
            for (int k = 0; k<3; k++){
                R_2[k] = v_F(R[k], r1, r12, a, R[k+3] - R[k]) * dtau/2 + R[k];
                R_2[k+3] = v_F(R[k+3], r2, r12, a, R[k+3] - R[k]) * dtau/2 + R[k];
            }

            //Diffusion part L_D
            for (int k = 0; k<6; k++){
                R_2[k] += sqrt(dtau) * randn(0., 1.);
            }

            r1 = r(R_2[0], R_2[1], R_2[2]);
            r2 = r(R_2[3], R_2[4], R_2[5]);
            r12 = r(R_2[3] - R_2[0], R_2[4] - R_2[1], R_2[5] - R_2[2]);

            for (int k = 0; k<3; k++){
                R[k] += v_F(R_2[k], r1, r12, a, R[k+3] - R[k]) * dtau;
                R[k] += v_F(R_2[k+3], r2, r12, a, R[k+3] - R[k]) * dtau;
            }
            ...
            // Weight function
            W[j] = exp( -(EL(R, r1, r2, r12, a) - ET[i]) * dtau);
